                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="page-title-box d-flex align-items-center justify-content-between">
                                    <h4 class="mb-0">
                                        @if($bdata['status'] == 'report')
                                        被举报评论
                                        @endif
                                        @if($bdata['status'] == 'uncheck')
                                        未审核评论
                                        @endif
                                        @if($bdata['status'] == 'approve')
                                        已审核评论
                                        @endif
                                        @if($bdata['status'] == 'all')
                                        全部评论
                                        @endif
                                        &nbsp;&nbsp; | &nbsp;&nbsp;
                                        <a href="/admin/comments/report" class="btn btn-outline-danger">被举报</a>
                                        &nbsp;&nbsp;
                                        <a href="/admin/comments/uncheck" class="btn btn-outline-primary">未审核</a>
                                        &nbsp;&nbsp;
                                        <a href="/admin/comments/approve" class="btn btn-outline-success">已审核</a>
                                        <!-- &nbsp;&nbsp;
                                        <a href="" class="btn btn-outline-danger">回收站</a> -->
                                        &nbsp;&nbsp;
                                        <a href="/admin/comments/all" class="btn btn-outline-info">全部</a>
                                    </h4>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="table-responsive mb-4">
                                            <table class="table table-centered table-nowrap mb-0">
                                                ...
                                                <tbody>
                                                    @foreach($bdata['comments'] as $data)
                                                    <tr tr_id="{{$data->id}}">
                                                        ...
                                                        <td class="center">
                                                            @if($data->status == 1)
                                                            <span class="hascheck">已审核</span>
                                                            @else
                                                            <span class="uncheck">未审核</span>
                                                            @endif
                                                        </td>
                                                        <td class="center">
                                                            @if($data->report == 0)
                                                            <span class="save">没</span>
                                                            @else
                                                            <span class="notSave">有</span>
                                                            @endif
                                                        </td>
                                                            ...
                                                            <ul class="list-inline mb-0">
                                                                ...
                                                                <div class="btn-group mr-1 mt-2">
                                                                    ...
                                                                    <div class="dropdown-menu" style="">
                                                                        @if($data->status == 1)
                                                                        <span class="dropdown-item checkBtn" onclick="CommentCheck({{$data->id}}, 0)">不通过审核</span>
                                                                        @else
                                                                        <span class="dropdown-item checkBtn" onclick="CommentCheck({{$data->id}}, 1)">通过审核</span>
                                                                        @endif

                                                                        @if($data->report == 1)
                                                                        <span class="dropdown-item checkBtn" onclick="commentReport({{$data->id}}, 0)">消除举报</span>
                                                                        @else
                                                                        <span class="dropdown-item checkBtn" onclick="commentReport({{$data->id}}, 1)">具有风险</span>
                                                                        @endif

                                                                        @if($data->remark == 'admin_reply')
                                                                        <!-- <span class="dropdown-item checkBtn editButton" id="{{$data->id}}" title="{{$data->title}}" data-toggle="modal" data-target="#editItem">修改回复</span> -->
                                                                        @endif
                                                                    </div>
                                                                </div>
                                    ...
                                    @else
                                    <div class="noContent">当前没有相关内容！</div>
                                    @endif
                                </div>
                            </div>
                        </div>
